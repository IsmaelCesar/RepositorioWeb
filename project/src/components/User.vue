  ...
  <nav class="navbar navbar-expand-md navbar-dark bg-secondary my-3" >
    <div class="container">
      ...
      <div class="collapse navbar-collapse text-center justify-content-end" id="navbarSecondarySupportedContent">
       ...
       <b>{{user_info != null ? user_info.nome : user}}</b>
      </div>
    </div>
  </nav>
...
export default {
  ...
  updated(){
    debugger
    if(this.user_info){
        if(this.user_info.obras.lenght != 0){
          this.$http.get('http://localhost:3000/obras?id_obra='+this.user_info.obras[0]).then(resp =>{
            var dadosRetornados
            dadosRetornados = resp.body[0]          
            this.user_obras[0] = dadosRetornados
          },
          resp=>{
